...
class PonentesController {
    public static function index(Router $router){
        ...
        if(!$pagina_actual || $pagina_actual > $paginacion->total_paginas()) header('Location: /admin/ponentes?page=1');
        ...
        if($_SERVER['REQUEST_METHOD'] === 'POST'){
            ...
            if(empty($alertas)){
                if(isset($nombre_imagen)){
                    if(file_exists($carpeta_imagenes . '/' . $ponente->imagen_actual . '.png')){
                        unlink($carpeta_imagenes . '/' . $ponente->imagen_actual . '.png');
                    }
                    if(file_exists($carpeta_imagenes . '/' . $ponente->imagen_actual . '.webp')){
